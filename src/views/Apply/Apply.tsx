...
export default function Apply() {
	...
	const applyList = useSelector((state: RootState) => state.checks.applyList).filter(
		(v) =>
	...
	useEffect(() => {
		if (_.isEmpty(applyList)) {
			dispatch(getApplyAction({ applicantid: usersInfos._id as string })).then((action) => {
				const { errcode, rets } = (action.payload as { [index: string]: unknown }).data as {
	...
	useEffect(() => {
		if (newsInfo.applicant) {
			dispatch(putRemindAction({ userid: usersInfos._id as string, applicant: false })).then(
				(action) => {
	...
	const onFinish = (values: FormInfos) => {
		...
		const applyList = {
			...
			approverid:
				...
				usersInfos.approver.find((item) => item.name === values.approvername)._id
		}
		dispatch(postApplyAction(applyList)).then((action) => {
			const { errcode } = (action.payload as { [index: string]: unknown }).data as {
				[index: string]: unknown
			}
			if (errcode === 0) {
